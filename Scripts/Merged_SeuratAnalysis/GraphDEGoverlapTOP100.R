...
## cell types
celltypes = c("DividingRadialGlia", "DLNeuron", "IntermediateProgenitor", "Neuron", "NPCtoNcells", "OuterRadialGlia", "RadialGlia", "ULNeuron");
## plot
pdf("GraphDEGoverlapTOP100.pdf");
for (i in 1:length(celltypes)) {
    ...
    ## plot
    DEG.list = list(DEG.C$gene[1:100], DEG.C_PR$gene[1:100], DEG.CKI_C.C$gene[1:100], DEG.P_PR.PR$gene[1:100]);
    DEG.names = c("C > P" , "C_PR > P_CKI" , "C > CKI", "PR > P");
    venn.diagram(
        x = DEG.list,
        category.names = DEG.names,
        filename = paste0(celltypes[i],'_control_like_DEG_TOP100_venn.png'),
        output=TRUE,
    ...
    pheatmap(output,display_numbers=T,main=paste0(celltypes[i]," control-like Jaccard index TOP100"));

    ## plot
    DEG.list = list(DEG.P$gene[1:100], DEG.P_CKI$gene[1:100], DEG.CKI_C.CKI$gene[1:100], DEG.P_PR.P$gene[1:100]);
    DEG.names = c("P > C" , "P_CKI > C_PR" , "CKI > C", "P > PR");
    venn.diagram(
        x = DEG.list,
        category.names = DEG.names,
        filename = paste0(celltypes[i],'_patient_like_DEG_TOP100_venn.png'),
        output=TRUE,
    ...
    pheatmap(output,display_numbers=T,main=paste0(celltypes[i]," patient-like Jaccard index TOP100"));
