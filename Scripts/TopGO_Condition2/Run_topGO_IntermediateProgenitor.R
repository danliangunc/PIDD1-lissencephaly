...
## DEG genes
DEG = read.csv("../Merged_SeuratAnalysis/IntermediateProgenitor_DEG_Condition2.csv",row.names=1);
## C_PR gene list
...
C_PR_GOdata <- new("topGOdata",ontology = "BP",description = "IPC Control + Patient Rescue Up Regulated Genes",allGenes = geneList,nodeSize = 10,annotationFun = annFUN.org,mapping = "org.Hs.eg.db");
...
geneList <- factor(as.integer(geneNames %in% myInterestingGenes));
names(geneList) <- geneNames;
P_CKI_GOdata <- new("topGOdata",ontology = "BP",description = "IPC Patient + Control KI Rescue Up Regulated Genes",allGenes = geneList,nodeSize = 10,annotationFun = annFUN.org,mapping = "org.Hs.eg.db");
...
write.csv(output, file = "Top_GO_IPC_results_Condition2.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_IPC_DEGene_Condition2.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
    bp = barplot(-log10(as.numeric(C_PRallRes$classicFisher)),main="IPC Control + Patient Rescue Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
    axis(2,at=bp,labels=C_PRallRes$Term,tick=FALSE,las=2,cex.axis=0.5);
    abline(v=-log(0.01),col="red",lwd=2,lty=1);

    bp = barplot(-log10(as.numeric(P_CKIallRes$classicFisher)),main="IPC Patient + Control KI Rescue Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
