...
## DEG genes
DEG = read.csv("../Merged_SeuratAnalysis/IntermediateProgenitor_DEG_Condition1.csv",row.names=1);
## C gene list
...
write.csv(output, file = "Top_GO_IPC_results_Condition1.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_IPC_DEGene_Condition1.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
    bp = barplot(-log10(as.numeric(CallRes$classicFisher)),main="IPC Control Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
