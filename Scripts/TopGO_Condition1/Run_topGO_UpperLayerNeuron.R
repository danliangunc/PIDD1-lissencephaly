...
## DEG genes
DEG = read.csv("../Merged_SeuratAnalysis/ULNeuron_DEG_Condition1.csv",row.names=1);
## C gene list
...
write.csv(output, file = "Top_GO_UpperLayerNeuron_results_Condition1.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_UpperLayerNeuron_DEGene_Condition1.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
