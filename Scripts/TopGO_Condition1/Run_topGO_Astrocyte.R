...
## DEG genes
AstrDEG = read.csv("../Merged_SeuratAnalysis/Astrocyte_DEG_Condition1.csv",row.names=1);
## C gene list
AstrDEG.C = AstrDEG[which(AstrDEG$cluster=="C"),];
AstrDEG.C = AstrDEG.C[which(AstrDEG.C$p_val_adj < 0.05 & AstrDEG.C$avg_log2FC > 0.1),];
## P gene list
AstrDEG.P = AstrDEG[which(AstrDEG$cluster=="P"),];
AstrDEG.P = AstrDEG.P[which(AstrDEG.P$p_val_adj < 0.05 & AstrDEG.P$avg_log2FC > 0.1),];
...
write.csv(output, file = "Top_GO_Astrocytes_results_Condition1.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_Astrocytes_DEGene_Condition1.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
