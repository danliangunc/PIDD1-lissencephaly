...
## DEG genes
AstrDEG = read.csv("../Merged_SeuratAnalysis/DividingRadialGlia_DEG_Condition3_C_CKI.csv",row.names=1);
## C gene list
...
geneList <- factor(as.integer(geneNames %in% myInterestingGenes));
names(geneList) <- geneNames;
CKI_GOdata <- new("topGOdata",ontology = "BP",description = "Dividing Radial Glia Control KI Up Regulated Genes",allGenes = geneList,nodeSize = 10,annotationFun = annFUN.org,mapping = "org.Hs.eg.db");
...
write.csv(output, file = "Top_GO_DividingRadialGlia_results_Condition3_C_CKI.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_DividingRadialGlia_DEGene_Condition3_C_CKI.pdf",height=8,width=6);
...
par(oma=c(2,10,1,1));
    ...
    bp = barplot(-log10(as.numeric(CKIallRes$classicFisher)),main="Dividing Radial Glia Control KI Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
...
## DEG genes
AstrDEG = read.csv("../Merged_SeuratAnalysis/DividingRadialGlia_DEG_Condition3_P_PR.csv");
## P gene list
...
geneList <- factor(as.integer(geneNames %in% myInterestingGenes));
names(geneList) <- geneNames;
PR_GOdata <- new("topGOdata",ontology = "BP",description = "Dividing Radial Glia Patient Rescue Up Regulated Genes",allGenes = geneList,nodeSize = 10,annotationFun = annFUN.org,mapping = "org.Hs.eg.db");
...
write.csv(output, file = "Top_GO_DividingRadialGlia_results_Condition3_P_PR.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_DividingRadialGlia_DEGene_Condition3_P_PR.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
    ...
    bp = barplot(-log10(as.numeric(PRallRes$classicFisher)),main="Dividing Radial Glia Patient Rescue Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
