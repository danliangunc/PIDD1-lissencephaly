...
## DEG genes
DEG = read.csv("DividingRadialGlia_DEG_PvsPR.csv",row.names=1);
## C gene list
...
write.csv(output, file = "Top_GO_DividingRadialGlia_results_PvsPR.csv",quote=F,row.names=F);
## plot
pdf(file="TopGO_DividingRadialGlia_DEGene_PvsPR.pdf",height=8,width=6);
par(oma=c(2,10,1,1));
    bp = barplot(-log10(as.numeric(CallRes$classicFisher)),main="Dividing Radial Glia PR Up Regulated Genes",horiz=TRUE,yaxt='n',yaxt='n',col="blue",xlab="-log10(P-value)",cex.main=0.5);
